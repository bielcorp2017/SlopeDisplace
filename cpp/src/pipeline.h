...
struct Config {
    ...
    // 0.05->0.01 한 번에 점프하면 finest 단계에서 대응거리가 좁아 남은 offset 을
    // 못 건너 fitness 가 폭락한다. 0.02 중간 스케일을 추가해 완만하게 수렴시킨다.
    std::vector<double> icp_voxels = {1.0, 0.4, 0.1, 0.05, 0.02, 0.01};
